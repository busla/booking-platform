...
output "agentcore_oauth2_provider_callback_urls" {
  description = <<-EOT
    OAuth2 Credential Provider callback URLs generated by AgentCore.
    After first apply, copy this URL to agentcore_oauth2_callback_url variable
    and run terraform apply again to complete OAuth2 setup.
  EOT
  value       = try(module.agentcore.identity.oauth2_provider_callback_urls, {})
  sensitive   = true
}

output "agentcore_oauth2_provider_names" {
  description = "OAuth2 Credential Provider names for use in agent code"
  value       = try(module.agentcore.identity.oauth2_provider_names, {})
  sensitive   = true
}

# Cognito AgentCore Client Outputs (for OAuth2 integration)
output "cognito_agentcore_client_id" {
  description = "Cognito User Pool Client ID for AgentCore OAuth2 (confidential client)"
  value       = module.cognito.agentcore_client_id
}

output "cognito_discovery_url" {
  description = "OIDC discovery URL for AgentCore OAuth2 configuration"
